...
int ehub_xhci_reset_device(struct usb_hcd *hcd)
{
	...
	/* Set dma_mask and coherent_dma_mask for DMA operations.
	 * USB devices inherit DMA from the USB host controller, so we need
	 * to use the bus's sysdev which has proper DMA capabilities.
	 */
	{
		struct device *dma_dev = hcd->self.sysdev;
		if (!dma_dev)
			dma_dev = hcd->self.controller;

		if (HCC_64BIT_ADDR(xhci->hcc_params) &&
				!dma_set_mask_and_coherent(dma_dev, DMA_BIT_MASK(64))) {
			xhci_dbg(xhci, "Enabling 64-bit DMA addresses. hcc=0x%0X\n",xhci->hcc_params);
		} else if (!dma_set_mask_and_coherent(dma_dev, DMA_BIT_MASK(32))) {
			xhci_dbg(xhci, "Using 32-bit DMA addresses. hcc=0x%0X\n", xhci->hcc_params);
		} else {
			xhci_dbg(xhci, "DMA mask setup skipped (virtual HCD)\n");
		}
	}
	...
	)
{
	...
	/* For DMA operations, use the parent USB bus's sysdev which has proper
	 * DMA capabilities. USB devices themselves don't have DMA ops set up.
	 */
	hcd->self.sysdev = DeviceContext->UsbContext.UsbDevice->bus->sysdev;
	dev_dbg(dev_ctx_to_dev(DeviceContext), "Using sysdev %s for DMA\n",
